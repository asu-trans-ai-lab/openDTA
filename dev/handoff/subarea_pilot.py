"""Subarea PM smoke pilot -- the MVP acceptance chain on agency-supplied data:
...
validation_eligible = false by construction.

NOTHING agency-specific is baked in. Data paths, the zone universe and the
first-through node id all come from the command line, so neither the data nor
the identifiers that describe it enter this repository. The dataset
conventions the pilot relies on:
- OMX zone index = TAZ - 1 (0-based); the zone universe is whatever --zones
  lists (internal TAZs plus boundary external stations);
- centroid/zone nodes are exactly those ids, all below --first-through-node;
  FTYPE=0 rows are centroid connectors (code 0);
...
Usage:
  python subarea_pilot.py --data-dir <dir> --work-dir <scratch>
      --zones 1,2,3 --first-through-node 1000 [--network-shp <name.shp>]
...
# set from the command line - see main(); never hard-coded here
ZONES = []
FIRST_THROUGH = 0
NETWORK_SHP = ""
...
def build_taplite_case(data_dir, case_dir):
    ...
    gdf = pyogrio.read_dataframe(os.path.join(data_dir, NETWORK_SHP))
    bpr = {}
    ...
    idx = [z - 1 for z in ZONES]   # 0-based index = TAZ - 1
    total = 0.0
...
def build_opendta_case(taplite_case, case_dir):
    ...
    with open(os.path.join(case_dir, "settings.yml"), "w", newline="\n") as f:
        f.write(f"""---
# Subarea PM smoke (generated by subarea_pilot.py -- agency-derived, do
...
def render_report(out_dir, work, total_demand, links):
    ...
    html = ("<!DOCTYPE html><html><head><meta charset='utf-8'>"
            "<title>Subarea PM smoke - OpenDTA DNL</title>"
            ...
            "<h1>Subarea PM smoke &mdash; TAPLite &rarr; OpenDTA DNL</h1>"
...
def main():
    ...
    ap.add_argument("--zones", required=True,
                    help="comma-separated zone ids (internal TAZs + external stations)")
    ap.add_argument("--first-through-node", required=True, type=int,
                    help="lowest non-centroid node id")
    ap.add_argument("--network-shp", default="",
                    help="network shapefile name inside --data-dir; defaults to the only .shp there")
    ...
    global ZONES, FIRST_THROUGH, NETWORK_SHP
    ZONES = [int(z) for z in args.zones.split(",") if z.strip()]
    FIRST_THROUGH = args.first_through_node
    NETWORK_SHP = args.network_shp
    if not NETWORK_SHP:
        shps = [f for f in os.listdir(args.data_dir) if f.lower().endswith(".shp")]
        if len(shps) != 1:
            sys.exit(f"--network-shp needed: {len(shps)} .shp files in --data-dir")
        NETWORK_SHP = shps[0]
