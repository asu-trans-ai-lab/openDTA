...
def manifest(case_id):
    """Run manifest for auditability: what exactly produced this report."""
    try:
        sha = subprocess.run(["git", "-C", os.path.join(ROOT, "..", ".."),
    ...
    rows = [
        ("OpenDTA commit", sha + " (+ working tree)"),
        ("executable sha256/16", _sha16(os.path.abspath(DEFAULT_EXE))),
        ("case settings sha256/16",
         _sha16(os.path.join(ROOT, "cases", case_id, "settings.yml"))),
        ("expected truth sha256/16",
         _sha16(os.path.join(ROOT, "expected", f"{case_id}_expected.csv"))),
        ("generator / oracle", "tools/generate_analytical_cases.py (fluid point queue, minute grid)"),
        ("simulation resolution", "6 sec"),
        ("threads", "1"),
        ("RNG", "not used (integer service path)"),
        ("entry clock origin", "07:00 = minute 0"),
        ("output sampling", "engine records AFTER the tick: sim row t == oracle minute t+1"),
        ("oracle alignment", "A_service(t) = A_entry(t - fftt); Q = A_service - D"),
        ("loading realization", "pre-S2: period volume in integer-minute batches "
                                "(oracle is exact for the intended piecewise-constant contract; "
                                "minute-batch transients are S2's target, not the queue engine's)"),
    ]
...
<details open><summary><b>Run manifest</b> (auditability)</summary>
<table>{manifest(case_id)}</table></details>
...
{svg_chart("Flow rates — arrivals vs discharge vs mu",
           ...
           n, "minute", "veh/h")}
{svg_chart("Waiting / travel time — the S0c terminal-link defect shows here",
           [("sim TT (reported)", sim["TT"][:n], "#d62728", ""),
            ("oracle TT = fftt + Q/mu", tt_oracle, "#2ca02c", "6,4"),
            ("sim waiting (reported)", w_sim, "#ff7f0e", ""),
            ("oracle waiting Q/mu", w_oracle, "#7f7f7f", "6,4")],
           n, "minute", "minutes")}
{speed_strip(sim, n)}
</body></html>"""
